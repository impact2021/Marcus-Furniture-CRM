jQuery(document).ready(function($) {
    ...
    if ($('.hs-crm-admin-wrap').length > 0) {
        ...
        $(document).on('change', '.hs-crm-status-radio-group input[type="radio"]', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        // Update status badge (Issue #5 fix - update colored text)
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        // Update radio buttons to match job type
                        if (isMovingHouse) {
                            $('#enquiry-type-moving').prop('checked', true);
                            $('#enquiry-type-pickup').prop('checked', false);
                        } else {
    ...
    // Mobile Enquiry Cards - Works on all admin pages
